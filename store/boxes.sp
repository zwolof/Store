...
public void eStore_SpawnBox(int client, BoxType boxtype){	
	if(IsClientInGame(client)) {
		...
		DispatchKeyValue(box.entity_id, "model", g_sBoxModels[iType]);
...
public void eStore_BoxPickedUp(int iEnt, int client) {
	...
	// 	eStore_Print(client, "This box belongs to \x04%N", owner);
	// 	// return;
	// }
...
void eStore_PrintBoxReward(int client, BoxType type) {
    ...
    Item item; char sRewards[512];

    // Item & Reward Formatting
    if(rewards.GetArray("item", item, sizeof(Item))) {
        if(eStore_UserHasItem(client, item.itemid)) {
            eStore_Print(client, "You already owned this item, instead we have given you \x04%d\x08 "...STORE_CREDITSNAME_LC..."!", item.price);
    ...
    if(credits > 0) {
        ...
        FormatEx(sRewards, sizeof(sRewards), "%s<br /><font color=\""...REWARD_COLOR_CREDITS..."\">%s</font>", sRewards, credits);
    }
